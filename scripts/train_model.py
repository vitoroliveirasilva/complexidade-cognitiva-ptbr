...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Executa a etapa de treinamento dos modelos.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = run_train_model(args.config)
        ...
        if args.debug:
            raise
        sys.stderr.write(f"Erro ao treinar os modelos: {exc}\n")
        return EXIT_ERROR


# Executa o treinamento dos modelos e retorna o resultado serializável
def run_train_model(config_path: str | Path = DEFAULT_CONFIG_PATH) -> Any:
