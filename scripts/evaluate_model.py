...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Executa a avaliação final do melhor modelo.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = run_evaluate_model(args.config)
        ...
        if args.debug:
            raise
        sys.stderr.write(f"Erro ao avaliar o modelo: {exc}\n")
        return EXIT_ERROR


# Executa a avaliação final do modelo e retorna o resultado serializável
def run_evaluate_model(config_path: str | Path = DEFAULT_CONFIG_PATH) -> Any:

    pipeline_cls = _load_pipeline_class()
    pipeline = pipeline_cls.from_config(config_path)
    return pipeline.evaluate_model()
