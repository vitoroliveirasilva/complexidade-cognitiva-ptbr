...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Prepara o dataset bruto em treino, validação e teste.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = run_prepare_dataset(args.config)
        ...
        if args.debug:
            raise
        sys.stderr.write(f"Erro ao preparar o dataset: {exc}\n")
        return EXIT_ERROR


# Executa a preparação do dataset e retorna o resultado serializável
def run_prepare_dataset(config_path: str | Path = DEFAULT_CONFIG_PATH) -> Any:
